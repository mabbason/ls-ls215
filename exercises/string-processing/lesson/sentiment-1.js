...
function sentiment(text) {
  text = parseText(text);
  let [positiveText, negativeText ] = 
      [ getWords('positive', text), getWords('negative', text) ];
  let [ numPosWords, numNegWords ] = [ positiveText.length, negativeText.length ];
...
function getWords(type, text) {
  return type === 'positive' ?
    text.filter(word => positiveWords.includes(word)):
    text.filter(word => negativeWords.includes(word));
}
